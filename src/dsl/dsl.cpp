...
bool peekPattern(Context& ctx, cstdstr& pattern){
  auto pos=0;
  for(auto& x : pattern){
    if(peekAhead(ctx,pos)!=x) break; else ++pos; }
...
std::pair<stdstr,Addr> str(Context& ctx){
  ...
  if(!ctx.eof && peek(ctx) == '"'){
    ...
    while(!ctx.eof){
      ...
      if(ch == '\\'){
        if(!advance(ctx))
          E_SYNTAX("Bad escaped char %c near line %d, col %d.", ch, _1(m), _2(m));
...
bool String::equals(DValue rhs) const{
  return is_same(rhs, this) && DCAST(String,rhs)->value == value; }
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
bool Number::match(const Number* rhs) const{
  return (isInt() && rhs->isInt()) ? getInt() == rhs->getInt()
                                   : a::fuzzy_equals(getFloat(), rhs->getFloat()); }
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
bool Number::equals(DValue rhs) const{
  return is_same(rhs, this) && match(DCAST(Number, rhs)); }
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
